...
async function handlePinterest(url) {
    try {
        ...
        if (pinInfo.status) {
            const result = pinInfo.result || {};
            
            if (result.success) {
                const mediaUrl = result.url || result.media || result.image || result.video;
                if (mediaUrl) {
                    return [{
    ...
    try {
        ...
        // Try to extract image/video URL from page HTML
        ...
        // Last resort: find any large jpg/png url in the page
