...
const FALLBACK_ORDER = ['GROQ', 'GEMINI', 'MISTRAL', 'OPENROUTER', 'CEREBRAS', 'SAMBANOVA', 'NVIDIA', 'DEEPSEEK', 'OPENAI'];
...
async function logAiUsage(provider, model, success, responseTime, userId = null, error = null) {
    try {
...
async function callAI(messages, options = {}) {
        ...
        while (retries <= maxRetries && !success) {
            try {
                ...
                if (content) {
                    ...
                    logAiUsage(apiName, config.model, true, responseTime, userId).catch(() => {});
    ...
    logAiUsage('ALL_FAILED', null, false, Date.now() - startTime, userId, errorMsg).catch(() => {});
    throw new Error(errorMsg);
}

module.exports = { API_CONFIGS, FALLBACK_ORDER, callAI, getErrorLog, getRecentErrors, logAiUsage };
