...
async function getBotmode(groupId) {
    try {
        const data = loadUserGroupData();
        return data.botmode?.[groupId] || 'public';
    } catch (error) {
        console.error('Error waktu ambil data botmode:', error);
        return 'public';
    }
}

async function setBotmode(groupId, mode) {
    try {
        const data = loadUserGroupData();
        if (!data.botmode) data.botmode = {};
        data.botmode[groupId] = mode;
        saveUserGroupData(data);
        return true;
    } catch (error) {
        console.error('Error waktu atur botmode:', error);
        return false;
    }
}
