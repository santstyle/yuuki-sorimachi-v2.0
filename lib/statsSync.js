...
const SYNC_INTERVAL = 30 * 60 * 1000; // 30 menit (stats lengkap)
const HEARTBEAT_INTERVAL = 5 * 60 * 1000; // 5 menit (ping online)
...
async function getStats() {
  ...
  return { total_users: totalUsers, total_groups: totalGroups, total_commands: totalCommands, total_banned: totalBanned };
}
...
async function sendHeartbeat() {
  if (!SUPABASE_URL || !SUPABASE_KEY) return;
  try {
    await axios.post(`${SUPABASE_URL}/rest/v1/stats`, { is_online: true, updated_at: new Date().toISOString() }, {
      headers: {
        apikey: SUPABASE_KEY,
        Authorization: `Bearer ${SUPABASE_KEY}`,
        'Content-Type': 'application/json',
...
function start() {
  ...
  log.info('Sync every 30 menit + heartbeat 5 menit');
}
