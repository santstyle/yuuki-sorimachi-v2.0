...
async function Antilink(msg, sock) {
	...
	try {
		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, {
					text: `@${senderNum} Tuan~ Jangan kirim link di grup ini~ Yuuki mohon~`,
					mentions: [sender]
				});
				break;
			...
			case 'kick':
				try {
					...
					await sock.sendMessage(jid, {
						text: `@${senderNum} Tuan telah Yuuki keluarkan karena mengirim link~ Maaf~`,
						mentions: [sender]
					});
				} catch (kickError) {
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					try {
						...
						await sock.sendMessage(jid, {
							text: `@${senderNum} Tuan telah Yuuki keluarkan karena mengirim link (${WARN_COUNT}/${WARN_COUNT} warning)~`,
							mentions: [sender]
						});
					} catch (kickError) {
						console.error('Gagal kick user:', kickError);
						await sock.sendMessage(jid, {
							text: `@${senderNum} Tuan~ Peringatan ${warningCount}/${WARN_COUNT} untuk mengirim link~`,
							mentions: [sender]
						});
					}
				} else {
