...
async function antideleteCommand(sock, chatId, message, match) {
    try {
        ...
        if (!match || match.toLowerCase() === 'status') {
            ...
            await sock.sendMessage(chatId, { text: `Tuan~ Saat ini anti-delete dalam keadaan *${status}*. Yuuki akan mengawasi setiap pesan yang dihapus... tidak ada yang bisa bersembunyi dari Yuuki, hehe~` });
            return;
        }
        ...
        if (match.toLowerCase() === 'on') {
            ...
            await sock.sendMessage(chatId, { text: 'Baik, Tuan~ Yuuki akan mengaktifkan mata ketiga Yuuki. Setiap pesan yang dihapus akan terlihat oleh Yuuki. Tidak ada rahasia yang bisa disembunyikan dari Tuan~' });
        } else if (match.toLowerCase() === 'off') {
            antideleteData.enabled = false;
            fs.writeFileSync('./data/antidelete.json', JSON.stringify(antideleteData, null, 2));
            await sock.sendMessage(chatId, { text: 'Baik, Tuan~ Yuuki akan menutup mata. Tapi ingat... Yuuki tetap tahu apa yang terjadi di bayang-bayang. Hanya saja Yuuki memilih untuk diam~' });
        } else {
